...
- **运行方式**：命令行编译运行 `ClinicApp` 主类，建议在编译/运行时加入 `mysql-connector-j` 驱动以启用 MySQL 双写；未提供驱动时系统会自动降级为仅写 CSV。
- **最新能力**：在原有 Insight 助理、专家会诊、病例库基础上，新增财务中心、库存盈亏、操作审计与 CSV ↔ MySQL 双写同步能力。
...
- **库存流水**：在 `InventoryManagementPanel` 中记录入库、出库与盘点调整，实时计算库存数量与金额。
...
- **财务中心**：`FinanceCenterPanel` 支持收费登记、付款状态变更、退款以及医保理赔提交流程。
- **统一交互体验**：医生端表格同样具备全局搜索、悬停预览与双击详情能力，加快定位关键信息。

### 2.4 管理员门户

- **审计日志**：`AuditLogPanel` 聚合支付、理赔、库存等关键操作记录，支持搜索过滤与时间排序，便于合规稽核。
- **运营监督**：管理员可以通过财务中心汇总四类金额（待支付、处理中、已支付、已退款），并结合库存流水排查异常。
- **数据核验**：配合 MySQL 双写数据，可在外部 BI 工具中对账，同时参考审计日志定位操作者与时间。
...
- **财务中心**：`PaymentService` + `FinanceCenterPanel` 支持收费登记、支付状态流转、退款处理及医保理赔提交流程，面板中实时展示多状态金额统计。
- **库存盈亏**：`InventoryService` 与 `InventoryManagementPanel` 协同，记录入库/出库/盘点调整并计算库存数量与成本，为药房补货和成本核算提供依据。
- **操作审计**：`AuditService` 将关键操作、目标实体、操作者角色与结果写入 `audit_logs.csv`，并提供管理员 UI 检索能力，满足合规稽核需求。
- **数据联动**：财务、库存、审计仓库均复用 `CsvDataStore` 机制，便于与既有业务表统一备份与迁移。

### 2.7 数据持久化与双写

- **CSV 主存储**：所有仓库依旧通过 `CsvDataStore` 读写本地 CSV，保持可视化编辑与离线演示能力。
- **MySQL 镜像**：`CsvDataStore.writeRecords` 触发 `CsvToMySqlMirror`，在单事务中对同名 MySQL 表执行 `DELETE + INSERT` 同步，保证写入原子性与数据一致性。
- **连接配置**：`MySqlConnectionManager` 读取环境变量或 JVM 参数（`CLINIC_DB_HOST`、`CLINIC_DB_PORT` 等）建立连接，默认指向 `localhost:3306` 的 `clinic` 库。
- **可选开关**：若缺少 MySQL 驱动或设置 `CLINIC_DB_SYNC_ENABLED=false`，镜像功能自动降级为只写 CSV，并输出告警日志，避免对原有流程造成影响。
...
└── ui
    ...
    │   ├── FinanceCenterPanel / InventoryManagementPanel
...
- `insurance_claims.csv`：`id|paymentId|insuranceType|coverageRatio|claimedAmount|approvedAmount|status|submittedAt|processedAt|notes`，记录医保理赔申请、审批与打款。
...
- **定时任务**：主界面内部维护 `javax.swing.Timer`，默认每 15 秒触发一次刷新；用户可通过工具栏手动刷新。
